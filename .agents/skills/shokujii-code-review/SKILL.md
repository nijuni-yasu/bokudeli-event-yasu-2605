...
description: Shokujiiプロジェクトのコーディング規約に従ってコードをレビューする。指摘は 🚨必須修正/🟡修正提案/👌修正不要（対応後は ✅対応済み）の4区分で review-comments-evaluate と共通（❌未対応は使わない）。PR紐づき時は pr-<番号>.md に RC 記録を追記。AGENTS.md に従い、3ファイル以上かつ細かなUI修正以外の実装完了時は lint-and-format 後に自動実行。🚨 は確認なしで自動修正（最大2周）。コード変更のレビュー依頼時にも使用。
---
