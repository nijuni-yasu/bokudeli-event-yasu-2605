...
describe('isGoogleAvatarHashCheckableUrl', () => {
  ...
  it('/a-/ パス（青いデフォルト等）は false', () => {
    expect(isGoogleAvatarHashCheckableUrl('https://lh3.googleusercontent.com/a-/ALV-UjExample=s100-c')).toBe(false)
  })
})

describe('isGoogleUnavailableAvatar', () => {
  it('/a-/ URL は fetch せず false を返す', async () => {
    const fetchMock = vi.fn()
    vi.stubGlobal('fetch', fetchMock)
    await expect(isGoogleUnavailableAvatar('https://lh3.googleusercontent.com/a-/ALV-UjExample=s100-c')).resolves.toBe(
      false,
    )
