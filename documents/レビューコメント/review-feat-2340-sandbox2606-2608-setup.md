...
|:----:|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| [x] | RC-1 | 3889247201 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>CORS に web.app を追加 |
| [x] | RC-2 | 3889247206 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>bootstrap メッセージ修正 |
| [x] | RC-3 | 3889249240 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>RC-1 同一 |
| [x] | RC-4 | 3889249246 | 🚨 必須修正 | ✅ 対応済み | 📌 スコープ内 | 🔒 セキュリティ | 📄 ドキュメントのみ | S<br>0004 必須化 |
| [x] | RC-5 | 3889249247 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>NUM 許可リスト |
| [x] | RC-6 | 3889249250 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>Slack コマンド名 |
| [x] | RC-7 | なし | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 📄 ドキュメントのみ | S<br>FIREBASERC 説明 |
| [x] | RC-8 | なし | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 📄 ドキュメントのみ | S<br>0004 実行リポジトリ |
| [x] | RC-9 | 3889269445 | 🚨 必須修正 | ✅ 対応済み | 📌 スコープ内 | 🔒 セキュリティ | 📄 ドキュメントのみ | S<br>Functions は 0004 後 |
| [x] | RC-10 | 3889269456 | 🚨 必須修正 | ✅ 対応済み | 📌 スコープ内 | 📑 仕様書 | 📄 ドキュメントのみ | S<br>Auth ユーザー用意 |
| [x] | RC-11 | 3889269458 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | 📏 規約 | 📄 ドキュメントのみ | S<br>RC 記録ブロック補完 |
| [x] | RC-12 | 3889269450 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | — | 🔧 微修正 | S<br>tfvars 先行作成 |
| [x] | RC-13 | 3889460556 | 🚨 必須修正 | ✅ 対応済み | 📌 スコープ内 | 📑 仕様書 | 📄 ドキュメントのみ | S<br>Auth UID 一致必須 |
| [x] | RC-14 | 3889460560 | 🟡 修正提案 | ✅ 対応済み | 📌 スコープ内 | 📏 規約 | 📄 ドキュメントのみ | S<br>サマリ表列ずれ修正 |
...
## 評価セッション（2026-08-30 22:16・review-comments-evaluate auto）

- **評価日時**: 2026-08-30 22:16 JST
- **評価者**: Cursor Agent（review-comments-evaluate auto・wait-ai-pr-review wake）
- **ブランチ名**: feat/2340-sandbox2606-2608-setup
- **PR**: #2343
- **REVIEW_REQUEST_SINCE**: 2026-08-30T13:06:50Z
- **Outdated 除外件数**: 0
- **レビュー非該当スキップ件数**: 0
- **手順 4a 自動修正**: RC-13・RC-14
...
**識別子**: RC-13（GitHub id: 3889460556）
...
**Auth UID を移行済み Firestore ドキュメントに合わせる**

既存指摘後に追加された現行 #8 でも、Console で任意のテストユーザーを作るだけでは移行データを使った検証になりません。partner は `partner/src/pages/login.vue:29` で認証した後、`base/src/stores/partner.ts:72` で Auth UID をそのまま `partners/{uid}` のキーにするため、移行済み partner と UID が一致しなければ店舗・イベント・注文を取得できません。また `pass_code.user_email` は直前の #5 で削除するので、これもログイン手段にはできません。既存 Firestore ドキュメントと同じ UID の Auth ユーザーを Admin SDK 等で作成するか、新規 Auth UID に対応する検証用 Firestore データを作る手順を明記してください。

**コメント要約**: Auth UID を移行 Firestore `partners/{uid}` と一致させる手順を Runbook に明記
...
**判断理由**: §6.3 #8 を「UID 一致必須」に改訂し、Admin SDK / Auth import 手順・#10/#7 検証項目も追記。

---


**識別子**: RC-14（GitHub id: 3889460560）

**レビュワー**: chatgpt-codex-connector[bot]

**指摘箇所**: `documents/レビューコメント/review-feat-2340-sandbox2606-2608-setup.md`
...
サマリ表のヘッダーは10列ですが、各行には GitHub id の後にレビュワー名が追加されて11列あるため、`Copilot` が評価列、`🟡 修正提案` がステータス列というように RC-1〜RC-12 の全項目が1列ずつずれて表示されます。セッション内の表も同様で、必須の評価・ステータス・PRスコープを正しく監査できないため、テンプレートどおりレビュワー列を除くか、ヘッダーと区切り行に対応する列を追加してください。

**コメント要約**: サマリ表から余分なレビュワー列を削除し列ずれを解消
...
**判断理由**: 通し RC 一覧・各セッション内サマリ表からレビュワー列を削除（10 列に統一）。
